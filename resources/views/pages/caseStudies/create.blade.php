...
<div class="container-fluid">
    <div class="row">
        <div class="col-lg-12">
            <div class="p-5">
                <div class="text-left">
                    <h1 class="">Add Case Study</h1>
                </div>
                <form action="{{ route('case-studies.store') }}" class="user" enctype="multipart/form-data" method="post">
                    @csrf
                    @if ($errors->any())
                    ...
                    <div class="row form-group">
                        <div class="mb-3 col-sm-6">
                            <input class="form-control" name="title" required placeholder="Case Study Title">
                        </div>
                        <div class="mb-3 col-sm-6">
                            <input class="form-control" name="slug" disabled required placeholder="Case Study Slug">
                        </div>
                        ...
                        <div class="mb-3 col-sm-12">
                            <label>Case Study Summary 
                                <small>Please write a short summary of the case study</small>
                            </label> 
                            <textarea class="form-control" name="summary" required></textarea>
                        </div>
                        <div class="mb-3 col-sm-12">
                            <label>Case Study Content</label> 
                            <textarea class="form-control" name="content" required id="case_content"></textarea>
                        </div>
                        <div class="mb-3 col-sm-12">
...
<script>
    jQuery(document).ready(function($) {
        ...
        if ($("#case_content").length > 0) {
            ...
            CKEDITOR.instances['case_content'].on('paste', function(event) {
                const pastedData = event.data.dataValue;
                if (isImage(pastedData)) {
                    const base64Image = toBase64(pastedData);
                    CKEDITOR.instances['case_content'].insertHtml(`<img src="${base64Image}" />`);
                }
            });
